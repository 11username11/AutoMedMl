...
async def get_optional_user(
        ...
        locale: Optional[str] = Cookie(None)
) -> Optional[dict]:
    user_data = {}
...
async def add_patient(
    ...
    try:
        ...
        msg_tpl = translations["welcome_message"].get(locale, translations["welcome_message"]["en"])
        welcome_message = msg_tpl.format(name=patient.name, surname=patient.surname)
...
async def models(current_user: dict = Depends(get_optional_user)):
    ...
    for model in models_cursor:
        models_list.append({
            "title": model["title"].get(locale, model["title"].get("en")),
            "technical_name": model["technical_name"],
            "category": model["category"].get(locale, model["category"].get("en")),
            "description": model["description"].get(locale, model["description"].get("en")),
...
async def send_research_to_chat(patient_id: str, research_id: str, current_user: dict = Depends(get_optional_user)):
    ...
    msg_tpl = translations["research_result"].get(current_user.get("locale", "en"),
                                                  translations["research_result"]["en"])
    message_text = msg_tpl.format(model=model, label=label, prob=prob)
